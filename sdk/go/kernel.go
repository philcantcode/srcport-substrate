...
func (k *Kernel) Register(m ModuleManifest) RegisterAck {
	...
	for _, c := range m.Provides {
		k.capabilities = append(k.capabilities, c)
...
func (k *Kernel) GetArtifact(ref ArtifactRef) (Artifact, error) {
	...
	if !ok {
		return Artifact{}, fmt.Errorf("%w: %s", ErrNotFound, ref.ID)
	}
	return a, nil
}
...
// Ledger returns a snapshot copy of the whole ledger, for verification/audit.
...
func (k *Kernel) DecideGate(d GateDecision) (GateDecision, error) {
	if d.Decision != DecisionApproved && d.Decision != DecisionRejected {
		return GateDecision{}, ErrNotADecision
	}
	k.mu.Lock()
	defer k.mu.Unlock()
	if _, ok := k.gates[d.RequestID]; !ok {
		return GateDecision{}, fmt.Errorf("%w: %s", ErrNotFound, d.RequestID)
	}
...
func (k *Kernel) AwaitGate(t GateTicket) (GateDecision, error) {
	...
	for {
		...
		if !ok {
			return GateDecision{}, fmt.Errorf("%w: %s", ErrNotFound, t.RequestID)
		}
...
func (k *Kernel) Snapshot() RegistrySnapshot {
	...
	snap := RegistrySnapshot{
		...
		Capabilities: append([]Capability(nil), k.capabilities...),
	}
	for _, m := range k.modules {
		snap.Modules = append(snap.Modules, m.manifest)
	}
